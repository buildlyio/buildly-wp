...
function scf_display_contact_form() {
	...
	$styles = !empty($custom) ? '<style>'. $custom .' .scf-confirm-checkbox { margin-top: 15px; } .scf-website3dhhsy3 { display: none; }</style>' : '';
	...
	if ( $honeypot ) {
		$scf_honeypot = '
			<fieldset class="scf-website3dhhsy3">
	...
	$scf_form = '<div id="simple-contact-form-wrap">' . $preform . $error . '
		<div id="simple-contact-form" class="scf">
			<form action="#simple-contact-form-wrap" method="post">
				...
				<fieldset class="scf-email">
					...
					$scf_subject . $scf_custom_fields . $scf_captcha . $scf_honeypot . $scf_message . '
...
function scf_confirm_checkbox() {
	...
	<div class="scf-confirm-checkbox">
		<input type="checkbox" required>  <?php echo $scf_options['scf_gdpr_message']; ?>

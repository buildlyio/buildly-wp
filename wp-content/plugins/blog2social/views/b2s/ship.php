...
if (isset($_GET['type']) && $_GET['type'] == 'draft' && isset($_GET['postId']) && (int) $_GET['postId'] > 0) {
    ...
    if ($wpdb->get_var("SHOW TABLES LIKE 'b2s_posts_drafts'") == 'b2s_posts_drafts') {
        $sql = $wpdb->prepare("SELECT data FROM `b2s_posts_drafts` WHERE `blog_user_id` = %d AND `post_id` = %d", (int) B2S_PLUGIN_BLOG_USER_ID, (int) $_GET['postId']);
                                        ...
                                        <form id="b2sNetworkSent" method="post">
                                            <div class="b2s-post-area col-md-9 del-padding-left">
                                                ...
                                                <div class="b2s-publish-area">
                                                    ...
                                                    <button type="submit" class="btn btn-success pull-right btn-lg b2s-submit-btn"><?php _e('Share', 'blog2social') ?></button>
                                                    <button type="button" class="btn btn-primary pull-right btn-lg b2s-draft-btn"><span class="b2s-loader-btn-ship b2s-loader-impulse"></span> <?php _e('Save as Draft', 'blog2social') ?></button>
                                                </div>
                                                <div class="navbar navbar-default navbar-fixed-bottom navbar-small b2s-footer-menu" style="display: block;">
                                                    <div class="b2s-publish-navbar-btn">
                                                        <button type="button" class="btn btn-primary btn-lg b2s-draft-btn-scroll"><span class="b2s-loader-btn-ship b2s-loader-impulse"></span> <?php _e('Save as Draft', 'blog2social') ?></button>
                                                        <button type="button" class="btn btn-success btn-lg b2s-submit-btn-scroll"><?php _e('Share', 'blog2social') ?></button>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <div class="modal-dialog modal-md">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            ...
                                            <h4 class="modal-title"><?php _e('Overwrite Draft', 'blog2social'); ?></h4>
                                        </div>
                                        <div class="modal-body">
                                            <b><?php _e('There is already a saved draft for this WordPress post or page. If you save a new draft it will overwrite the old draft.  Are you sure you want to overwrite your draft?', 'blog2social') ?></b>
                                        </div>
                                        <div class="modal-footer">
                                            <button class="btn btn-default" data-dismiss="modal"><?php _e('NO', 'blog2social') ?></button>
                                            <button class="btn btn-primary b2s-save-draft-confirm-btn"><?php _e('YES', 'blog2social') ?></button>
                                        </div>
                                    </div>
                                </div>
                            </div>

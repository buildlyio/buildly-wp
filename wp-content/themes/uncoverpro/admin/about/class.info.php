...
if ( ! class_exists( 'uncoverpro_Info' ) ) {
	...
	class uncoverpro_Info {
		 ...
		 * Single instance.
		 *
		 * @var uncoverpro_Info $instance Instance object.
		 */
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof uncoverpro_Info ) ) {
				self::$instance = new uncoverpro_Info;
		...
		public function configure() {
			...
			$this->menu_name     = isset( $this->config['menu_name'] ) ? $this->config['menu_name'] : sprintf( esc_html__( '%s Info', 'uncoverpro' ), $this->theme_name );
			$this->page_name     = isset( $this->config['page_name'] ) ? $this->config['page_name'] : sprintf( esc_html__( '%s Info', 'uncoverpro' ), $this->theme_name );
			$this->tabs          = isset( $this->config['tabs'] ) ? $this->config['tabs'] : array();
			$this->page_slug     = $this->theme_slug . '-info';
			$this->page_url     = admin_url( 'themes.php?page=' . $this->page_slug );
			$this->notice        = '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s. To fully take advantage of the best our theme can offer please make sure you visit theme info page.', 'uncoverpro' ), esc_html( $this->theme_name ) ) . '</p><p><a href="' . esc_url( $this->page_url ) . '" class="button button-primary">' . sprintf( esc_html__( 'Get started with %1$s', 'uncoverpro' ), $this->theme_name ) . '</a><a href="#" class="btn-dismiss" data-userid="' . esc_attr( get_current_user_id() ) . '" data-nonce="' . esc_attr( wp_create_nonce( 'uncoverpro_dismiss_nonce' ) ) . '">' . esc_html__( 'Dismiss this notice', 'uncoverpro' ) . '</a></p>';
		}
		...
		public function hooks() {
			...
			// Dismiss AJAX.
			add_action( 'wp_ajax_uncoverpro_dismiss', array( $this, 'dismiss_callback' ) );
			add_action( 'wp_ajax_nopriv_uncoverpro_dismiss', array( $this, 'dismiss_callback' ) );
		}
		...
		public function render_tabs() {
			...
			$allowed_html = uncoverpro_allowed_html();
		...
		public function render_current_tab_content() {
			...
			if ( method_exists( $this, $method ) ) {
				...
				printf( esc_html__( '%s() method does not exist.', 'uncoverpro' ), esc_html($method) );
			}
		}
		...
		public function useful_plugins() {
			...
			<div class="wt-section wt-section-useful-plugins">
				...
				<?php if ( true !== $tgmpa_complete ) : ?>
					<a href="<?php echo esc_url( $tgmpa_url ); ?>" class="button button-primary"><?php esc_html_e( 'Manage Plugins', 'uncoverpro' ); ?></a>
				<?php endif; ?>
		...
		public function assets( $hook ) {
			...
			wp_enqueue_style( 'uncoverpro-info', get_template_directory_uri() . '/admin/about/css/info' . $min . '.css', array(), '1.0.0' );
			wp_enqueue_script( 'uncoverpro-info', get_template_directory_uri() . '/admin/about/js/info' . $min . '.js', array(), '1.0.0' );
			}
		...
		public function display_admin_notice() {
			...
			$dismiss_status = get_user_meta( $user_id, 'uncoverpro_dismiss_status', true );
			?>
		...
		public function dismiss_callback() {
			...
			if ( false === wp_verify_nonce( $wpnonce, 'uncoverpro_dismiss_nonce' ) ) {
				wp_send_json( $output );
			}

			update_user_meta( $userid, 'uncoverpro_dismiss_status', 1 );

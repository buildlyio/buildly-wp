 ...
 * Styles Enqueue
 */
function blackrider_add_stylesheet() {
    if (!is_admin()) {
        wp_enqueue_style('blackrider_stylesheet', get_template_directory_uri() . "/style.css", '', '', 'all');
    } 
}
add_action('init', 'blackrider_add_stylesheet');

/*
* Redirect to about us page.
*/
if (is_admin() && isset($_GET['activated']) && $pagenow == "themes.php")
wp_redirect('themes.php?page=black-rider-welcome');
...
function black_rider_tracking_admin_notice() {
    global $current_user;
    $user_id = $current_user->ID;
    /* Check that the user hasn't already clicked to ignore the message */
    if (!get_user_meta($user_id, 'wp_email_tracking_ignore_notice')) {
        ?>
        <div class="updated um-admin-notice"><p><?php _e('Allow Black-Rider theme to send you setup guide? Opt-in to our newsletter and we will immediately e-mail you a setup guide along with 20% discount which you can use to purchase any theme.', 'black-rider'); ?></p><p><a href="<?php echo get_template_directory_uri() . '/functions/smtp.php?wp_email_tracking=email_smtp_allow_tracking'; ?>" class="button button-primary"><?php _e('Allow Sending', 'black-rider'); ?></a>&nbsp;<a href="<?php echo get_template_directory_uri() . '/functions/smtp.php?wp_email_tracking=email_smtp_hide_tracking'; ?>" class="button-secondary"><?php _e('Do not allow', 'black-rider'); ?></a></p></div>
        <?php
    }
}

add_action('admin_notices', 'black_rider_tracking_admin_notice');
